...
import { getManualTrade } from "@/lib/db/manualTrades";
...
export async function GET(request, { params }) {
  ...
  const [snapshot, outcome, manualTrade] = await Promise.all([
    getSnapshot(params.id), getOutcome(params.id), getManualTrade(params.id),
  ]);

  return NextResponse.json({ success: true, prediction, snapshot, outcome, manualTrade });
}
