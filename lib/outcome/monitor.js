import { getSpotCandles } from "../bitget/spot.js";
import { getFuturesCandles } from "../bitget/futures.js";
...
export async function fetchOutcomeCandles({
  ...
  //
  // WAJIB dibungkus: dulu baris ini telanjang, jadi symbol yang ditolak Bitget
  // (mis. "4USDT" -> HTTP 400) melempar keluar sampai ke worker. Barisnya tidak
  // tersentuh sama sekali — termasuk next_check_at yang tetap bernilai lama —
  // sehingga selamanya berada di kepala antrean "paling telat dulu" dan memakan
  // slot di SETIAP panggilan worker. Satu symbol rusak jadi pajak permanen.
  if (!raw || raw.length === 0) {
    try {
      ({ candles: raw } = await request({}));
      if (raw && raw.length > 0) fetchError = null;
    } catch (err) {
      fetchError = err?.message || String(err);
      raw = [];
    }
  ...
  return { candles, timeframe, windowEnd, complete, requestedLimit: limit, fetchError };
}
