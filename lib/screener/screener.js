...
import { filterUniverseByAssetClass, classifyAsset } from "./universe";
...
function buildUniverse(symbols, tickers, config) {
  ...
  // Saham/ETF/komoditas ter-tokenisasi TIDAK dibuang secara default.
  //
  // Hipotesis awal: aset ini tidak berdagang 24/7, jadi ATR-nya mengecil saat
  // bursa tutup dan menghasilkan stop loss mikroskopis. Tapi backtest di data
  // 2026-08-25..09-03 (n=603) TIDAK mendukung pembuangannya — kripto justru
  // sedikit lebih buruk (net -0.366% vs -0.200%), dan selisihnya tidak
  // signifikan. Masalah stop sempit ternyata sudah ditangani secara mekanis
  // oleh batas MIN_STOP_ATR_MULTIPLIER di config/risk.js.
  //
  // Jadi: kelas aset tetap DIKLASIFIKASI dan dicatat per setup supaya bisa
  // dikalibrasi terpisah, tapi tidak disaring sampai ada bukti yang mendukung.
  // Set EXCLUDE_TOKENIZED_ASSETS: true untuk mengujinya.
  const { kept, excludedByClass } = filterUniverseByAssetClass(tradable, {
    allowTokenized: config.EXCLUDE_TOKENIZED_ASSETS !== true,
  });

  return { universe: kept, excludedByClass };
}
...
function buildScreenerEntry(item, candles, timeframe, config, btcContext) {
  ...
  const {
    rawScore, penalty, finalScore, coverage, liquidityFactor, penaltyFactor, rejectReason, scoreVersion,
  } = calculateScreenerScore(
    { momentumScore, volumeScore, liquidityScore, volatilityScore, breakoutScore },
    penalties,
    config,
    // R:R dari harga SEKARANG ikut jadi gerbang, bukan cuma salah satu
    // komponen entryQuality. Setup dengan R:R di bawah ambang tidak diberi
    // skor sama sekali (finalScore null) sehingga tidak pernah ikut dicatat.
  ...
  return {
    ...
    penalty,
    penalties,
    // Diagnostik scoring v3 — ikut disimpan supaya kalibrasi berikutnya bisa
    // memisahkan "skor rendah" dari "setup ditolak gerbang".
    scoreVersion,
    scoreCoverage: coverage,
    liquidityFactor,
    penaltyFactor,
    scoreRejectReason: rejectReason,
    assetClass: classifyAsset(item.symbol),
...
/**
 * @param {boolean} record  Catat setup yang memenuhi syarat ke database?
 *
 * Default FALSE — ini perubahan perilaku yang disengaja. Dulu pencatatan ikut
 * menempel di setiap kali halaman Screener/Dashboard dibuka, lewat panggilan
 * fire-and-forget yang tidak pernah ditunggu. Dua akibatnya:
 *
 *   1. Riwayat hanya bertambah saat aplikasi kebetulan dibuka. Hari-hari Anda
 *      tidak membuka aplikasi = lubang di data, dan sampel jadi condong ke jam
 *      Anda online — racun untuk kalibrasi, yang butuh sampel merata 24 jam.
 *   2. Vercel boleh membekukan proses begitu response terkirim, jadi sebagian
 *      pencatatan bisa terpotong di tengah jalan tanpa jejak.
 *
 * Sekarang pencatatan hanya dilakukan oleh /api/worker/record-setups yang
 * dipanggil scheduler, dan SELALU ditunggu sampai selesai.
 */
export async function runScreener({ mode, timeframe, config = SCREENER_CONFIG, record = false }) {
  const isFutures = mode === "futures";
  ...
  const { universe, excludedByClass } = buildUniverse(symbolsResult.symbols, tickersResult.tickers, config);
  ...
  const results = await mapWithConcurrency(candidates, config.CONCURRENCY_LIMIT, async (item) => {
    try {
      ...
      if (record) {
        // DITUNGGU, bukan fire-and-forget. Hasilnya dibawa keluar supaya
        // pemanggil bisa melaporkan berapa yang tercatat dan kenapa sisanya tidak.
        entry.recordResult = await autoRecordIfEligible({
          mode, timeframe, entry, btcMomentumLabel: btcContext.momentumLabel, config,
        });
      }
      return entry;
    } catch (err) {
  ...
  const recordSummary = record ? summarizeRecords(results) : null;
  ...
  return {
    ...
    excludedByAssetClass: excludedByClass,
    btcMomentumLabel: btcContext.momentumLabel,
    results: results.filter(Boolean).sort((a, b) => (b.screenerScore ?? -1) - (a.screenerScore ?? -1)),
  };
}

/** Ringkasan pencatatan: berapa tercatat, dan alasan sisanya dilewati. */
function summarizeRecords(results) {
  const summary = { recorded: 0, control: 0, duplicate: 0, skipped: 0, errors: [], skipReasons: {} };

  for (const entry of results) {
    const r = entry?.recordResult;
    if (!r) continue;
    if (r.status === "RECORDED") {
      summary.recorded += 1;
      if (r.isControl) summary.control += 1;
    } else if (r.status === "DUPLICATE") {
      summary.duplicate += 1;
    } else if (r.status === "ERROR") {
      // Maksimal 5 contoh — cukup untuk mendiagnosa, tidak membanjiri response.
      if (summary.errors.length < 5) summary.errors.push({ symbol: r.symbol, error: r.error });
    } else {
      summary.skipped += 1;
  ...
  summary.errorCount = results.filter((e) => e?.recordResult?.status === "ERROR").length;
  return summary;
}

/** Selalu TANPA pencatatan: hasilnya di-cache, jadi tidak boleh punya efek samping. */
export async function runScreenerCached({ mode, timeframe, config = SCREENER_CONFIG }) {
  ...
  const result = await runScreener({ mode, timeframe, config, record: false });
