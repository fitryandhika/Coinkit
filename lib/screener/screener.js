...
async function fetchBtcContext({ mode, timeframe, config }) {
  ...
  try {
    ...
    return { closes, momentumLabel: describeMomentum(momentum.m1, config) };
  } catch (err) {
...
function buildScreenerEntry(item, candles, timeframe, config, btcContext) {
  ...
  // Arah + level entry/SL/TP — untuk kartu Screener dan bahan Screener History
  const direction = inferDirection({ momentumLabel, breakoutStatus: breakout.status });
  ...
  const btcReturns = toReturns(btcContext.closes);
...
export async function runScreener({ mode, timeframe, config = SCREENER_CONFIG }) {
  ...
  const results = await mapWithConcurrency(candidates, config.CONCURRENCY_LIMIT, async (item) => {
    try {
      ...
      autoRecordIfEligible({ mode, timeframe, entry }).catch(() => {});
      return entry;
    } catch (err) {
      return {
        symbol: item.symbol,
