...
/** Sumber data sekarang: hasil Screener (auto-record), bukan lagi AI Assistant.
 * Field confidence & market_regime sudah tidak pernah diisi (sumbernya dihapus),
 * jadi tidak dihitung di sini — daripada tampilkan grup "UNKNOWN" yang kosong. */
export async function buildPerformanceReport() {
  ...
  const predictions = rows.map((r) => {
    ...
    return {
      ...
      outcome: r.outcome,
      status: r.status,
      maximum_gain_pct: r.maximum_gain_pct,
      maximum_drawdown_pct: r.maximum_drawdown_pct,
      maximum_r: r.maximum_r,
  ...
  const scoreCalibration = Object.fromEntries(
    Object.entries(scoreBuckets).map(([b, r]) => [b, { count: r.length, averageMaxGainPct: average(r.map((x) => x.maximum_gain_pct)) }])
  );
  ...
  const scatterData = predictions.map((p) => ({
    ...
    maxGain: p.maximum_gain_pct,
    maxDrawdown: p.maximum_drawdown_pct,
  }));
