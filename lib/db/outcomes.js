...
/** Kapan worker terakhir kali benar-benar MENYELESAIKAN sebuah evaluasi.
 * Bukan "kapan worker dipanggil" — panggilan yang gagal auth atau timeout tidak
 * meninggalkan jejak, sedangkan baris ini hanya bergerak kalau kerjanya jadi. */
export async function getLastEvaluationEndedAt() {
  const supabase = getSupabaseClient();
  const { data } = await supabase
    .from("prediction_outcomes")
    .select("evaluation_ended_at")
    .not("evaluation_ended_at", "is", null)
    .order("evaluation_ended_at", { ascending: false })
    .limit(1)
    .maybeSingle();
  return data?.evaluation_ended_at ?? null;
}
