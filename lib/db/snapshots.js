...
export async function createSnapshot(predictionId, snapshot) {
  ...
  const { error } = await supabase.from("prediction_snapshots").insert({
    ...
    // Bug lama: autoRecord mengirim `volume` & `volumeRatio` sebagai angka datar,
    // tapi baris ini membacanya sebagai objek -> keduanya selalu tersimpan null.
    volume: typeof snapshot.volume === "number" ? snapshot.volume : snapshot.volume?.currentVolume ?? null,
    volume_ratio: snapshot.volumeRatio ?? snapshot.volume?.volumeRatio ?? null,
    ...
    // --- kelayakan harga entry ---
    entry_score: snapshot.entryScore ?? null,
    entry_label: snapshot.entryLabel ?? null,
    risk_reward: snapshot.riskReward ?? null,
    chase_gap_pct: snapshot.chaseGapPct ?? null,
    extension_atr: snapshot.extensionAtr ?? null,
  });
