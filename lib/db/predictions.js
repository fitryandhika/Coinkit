...
export async function createPrediction(data) {
  ...
  const { error } = await supabase.from("ai_predictions").insert({
    ...
    // Menandai setup ini dicatat & dievaluasi dengan aturan versi berapa. Data
    // sebelum perbaikan (v1) tidak sebanding dengan data sesudahnya.
    ruleset_version: data.rulesetVersion ?? CURRENT_RULESET_VERSION,
    status: "PENDING",
  });
...
/**
 * Prediction yang masih PENDING, PALING LAMA DULU.
 *
 * `.order()` bukan hiasan: tanpa itu Supabase bebas mengembalikan baris yang
 * sama setiap kali dan sebagian setup tidak pernah dievaluasi. Sekarang dipakai
 * hanya sebagai jaring pengaman untuk prediction yang baris outcome-nya belum
 * terbentuk — antrean utamanya ada di listDueOutcomes().
 */
export async function listDuePredictions({ statuses = ["PENDING"], limit = 200 } = {}) {
  const supabase = getSupabaseClient();
 ...
 * punya SATU prediction PENDING AKTIF.
 *
 * `timestamp` dan `evaluation_horizon` ikut diambil karena "aktif" sekarang
 * berarti horizonnya belum habis — bukan sekadar status PENDING. Lihat
 * duplicateBlocks() di lib/screener/recordPolicy.js untuk alasannya. */
export async function findActivePrediction({ symbol, market, timeframe }) {
  const supabase = getSupabaseClient();
  const { data, error } = await supabase
    .from("ai_predictions")
    .select("id, timestamp, evaluation_horizon")
    .eq("symbol", symbol)
    .eq("market", market)
...
/** Untuk kartu status di halaman Kalibrasi: kapan setup terakhir dicatat, dan
 * berapa banyak PENDING yang horizonnya sudah jelas lewat (indikasi worker mati). */
export async function getRecordingHealth({ stalePendingHours = 72 } = {}) {
  const supabase = getSupabaseClient();
  ...
  const staleBefore = new Date(Date.now() - stalePendingHours * 60 * 60 * 1000).toISOString();
  ...
  return {
    lastRecordedAt: last?.timestamp ?? null,
    pendingCount: pendingCount ?? null,
    stalePendingCount: stalePendingCount ?? null,
  };
}
