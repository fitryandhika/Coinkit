...
export async function listPredictions({ symbol, decision, status, limit = 50 } = {}) {
  const supabase = getSupabaseClient();
...
export async function updateUserAction(id, action) {
  const supabase = getSupabaseClient();
  const { error } = await supabase.from("ai_predictions").update({ user_action: action }).eq("id", id);
  if (error) throw new Error(`Gagal update user action: ${error.message}`);
}
